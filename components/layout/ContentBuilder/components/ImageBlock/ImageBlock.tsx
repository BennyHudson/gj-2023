  ...
  return (
    <Styled.ImageBlock imageSize={imageSize}>
      {imageSize === 'standard--full' ? 
        <Image src={featuredImageUrl(image.sourceUrl)} width={750} height={750} alt={image.caption ? image.caption.replace(/<\/?[^>]+(>|$)/g, '').trim() : ''} />
        :
        <Image src={featuredImageUrl(image.sourceUrl)} width={750} height={1000} alt={image.caption ? image.caption.replace(/<\/?[^>]+(>|$)/g, '').trim() : ''} />
      } 
      {image.caption && 
